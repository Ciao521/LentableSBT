...
// async function toggleRentability() {
//   // transaction のデータを作成
//   const data = nftContract.methods.toggleRentability(tokenURI).encodeABI(); //mintでNFTを発行する際のWALLET_ADDRESSは発行先

//   // transaction に用いるために予想される gas と gasPrice を取得
//   const gasP = nftContract.methods.toggleRentability(tokenURI).estimateGas();
...
async function toggleRentability(tokenURI) {
  try {
      const txReceipt = await contract.methods.toggleRentability(tokenURI).send({
          from: web3.eth.defaultAccount,
          gas: 500000
      });
      console.log('Transaction successful: ', txReceipt);
  } catch (error) {
      console.error('Error toggling rentability: ', error);
  }
}
toggleRentability();
