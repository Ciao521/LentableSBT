...
async function _beforeTokenTransfer() {
  // transaction のデータを作成
  const data = nftContract.methods._beforeTokenTransfer(WALLET_ADDRESS,0).encodeABI(); //mintでNFTを発行する際のWALLET_ADDRESSは発行先

  // transaction に用いるために予想される gas と gasPrice を取得
  const gasP = nftContract.methods._beforeTokenTransfer(WALLET_ADDRESS, 0).estimateGas();
