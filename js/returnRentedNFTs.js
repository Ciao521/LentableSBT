...
async function returnRentedNFTs() {
  // transaction のデータを作成rentOutMultipl
  const data = nftContract.methods.returnRentedNFTs(WALLET_ADDRESS,0).encodeABI(); //mintでNFTを発行する際のWALLET_ADDRESSは発行先

  // transaction に用いるために予想される gas と gasPrice を取得
  const gasP = nftContract.methods.returnRentedNFTs(WALLET_ADDRESS, 0).estimateGas();
