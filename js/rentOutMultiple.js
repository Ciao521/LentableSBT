...
async function rentOutMultiple() {
  // transaction のデータを作成
  const data = nftContract.methods.rentOutMultiple(0,WALLET_ADDRESS,1692845668663).encodeABI(); //mintでNFTを発行する際のWALLET_ADDRESSは発行先

  // transaction に用いるために予想される gas と gasPrice を取得
  const gasP = nftContract.methods.rentOutMultiple(0,WALLET_ADDRESS).estimateGas();
